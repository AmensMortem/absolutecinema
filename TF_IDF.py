...
#############################################
# Model Architecture — MLP (3 скрытых слоя)
#############################################
...
train_losses = []
train_f1_scores = []  # FIX: было train_accuracies — список никогда не заполнялся
train_accuracies = []  # Exact match accuracy (все жанры угаданы точно)
...
for epoch in range(EPOCHS):
    ...
    # FIX: убран дублирующий импорт f1_score внутри цикла
    micro = f1_score(all_true, all_preds, average="micro")
    macro = f1_score(all_true, all_preds, average="macro")

    # Exact match accuracy: строка считается верной только если все жанры угаданы точно
    all_preds_np = np.array(all_preds)
    all_true_np  = np.array(all_true)
    accuracy = np.mean(np.all(all_preds_np == all_true_np, axis=1))

    train_f1_scores.append(micro)  # FIX: сохраняем micro F1 для истории обучения
    ...
    scheduler.step(epoch_loss)  # FIX: scheduler.step() вызывался, но нигде не использовался
...
metrics_df = pd.DataFrame({
    ...
    "accuracy": train_accuracies  # Exact match accuracy на val
})
...
# На инференсе и тесте всегда используем best_t, найденный на валидации.

#############################################
# ОЦЕНКА КАЧЕСТВА НА ТЕСТЕ (с best_t)
#############################################
...
print("\n=== Отчёт классификации (тестовая выборка, порог=best_t) ===")
...
def predict_genres(text: str) -> tuple:
    ...
    with torch.no_grad():
        ...
        preds = (torch.sigmoid(outputs) > best_t).int().cpu().numpy()  # используем best_t
