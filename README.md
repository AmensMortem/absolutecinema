...
1. Download all the code from this repository (better do it as .zip) 
2. Open it as a project in any code editor
3. Install all the packages required (all of them are written in requirements.txt)
4. Run the main.py file in the root folder of project.

Since there's no model uploaded on GitHub (files are too heavy to do that), first the model will be trained on your device, so it will take some time. At the first time you run the app, the model will be saved, and next time just loaded from your files, so it won't take that long.
