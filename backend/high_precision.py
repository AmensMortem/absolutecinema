...
DATASET_DIR = "IMDb Movie Genre Classification"  # Folder with the TMDB dataset - place it next to the script
...
# PYTORCH DATASET fastText gives dense matrices (10k * 300) and convert to tensor immediately
...
# Separately, we are looking for the threshold for maximum exact match accuracy
best_t_acc = 0.5
best_acc = 0.0
...
# We are looking for a threshold for maximum precision with recall >= 0.4
# It's a balance: don't lose too much recall, but the precision is high

best_t_prediction = 0.5
best_prediction = 0.0

for t in np.arange(0.3, 0.95, 0.02):
    preds = (val_probs > t)
    rec = recall_score(val_true, preds, average="micro", zero_division=0)
    if rec < 0.35:  # don't let recall fall below 35%
        break
    prec = precision_score(val_true, preds, average="micro", zero_division=0)
    if prec > best_prediction:
        best_prediction = prec
        best_t_prediction = t

print(f"\n BEST THRESHOLD (High Precision): {round(best_t_prediction, 2)}")
print(f" BEST PRECISION (val): {round(best_prediction, 4)}")
...
# For inference and testing we use best_t_prec - high precision.
...
def evaluate(threshold, label):
    ...
    with torch.no_grad():
        for X_batch, y_batch in test_loader:
            ...
            with torch.amp.autocast(device_type=device.type, enabled=use_amp):
                outputs = model(X_batch)
            predictions_list.extend((torch.sigmoid(outputs) > threshold).cpu().numpy())
    ...
    print(f"\n=== {label} (threshold={round(threshold, 2)}) ===")
...
all_preds_f1 = evaluate(best_t, "Micro F1 mode")
all_preds_prec = evaluate(best_t_prediction, "High Precision Mode")
all_preds_acc = evaluate(best_t_acc, "Exact Match Mode")

print("\n=== Detailed report (High Precision mode) ===")
